...
@Component({
  ...
  templateUrl: "app/comp/import/import.html",
...
export class ImportComponent implements OnInit {
  ...
  saveAllTx() {
    let toSaveList: Array<TxFormData> = this.txFormDataList.filter(txFormData => {
        if (txFormData.categoryLink.categoryId) { return true; } else { return false; } }
      );
    toSaveList.forEach(txFormData => {
      ...
      (function(comp: ImportComponent, inComptaDate: Date, txFormData: TxFormData) {
        comp._categoryRestService.existsCategoryForYear(txFormData.categoryLink.categoryId,
            inComptaDate.getFullYear()).subscribe(category => {
          if (category) {
            ...
            console.error("Error adding tx with ref ", txFormData.tx.ref, "category",
                txFormData.categoryLink.categoryId, "not available for year", inComptaDate.getFullYear());
          }
        });
      })(this, comptaDate, txFormData);

    });
    this.txFormDataList = this.txFormDataList.filter(txFormData => {
      if (txFormData.categoryLink.categoryId) { return false; } else { return true; };
    });
    this.populateTxToDisplay();
  }
}
